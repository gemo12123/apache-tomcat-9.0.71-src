...
public abstract class LifecycleBase implements Lifecycle {
     ...
     * StandardServer中的监听器是在 Catalina 对象解析 server.xml 文件时就已经创建好并加到集合中的
     */
    ...
    public final synchronized void init() throws LifecycleException {
        // 非NEW状态禁止调用init()，否则直接报错
        if (!state.equals(LifecycleState.NEW)) {
            invalidTransition(Lifecycle.BEFORE_INIT_EVENT);
        }

        try {
            // 初始化逻辑之前，将状态变更为`INITIALIZING`，并触发相关事件
            setStateInternal(LifecycleState.INITIALIZING, null, false);
            // 初始化，该方法为一个abstract方法，需要组件自行实现
            initInternal();
            // 初始化完成之后，状态变更为`INITIALIZED`，并触发相关事件
    ...
    public final synchronized void start() throws LifecycleException {
        // `STARTING_PREP`、`STARTING`和`STARTED`时，将忽略start()逻辑直接跳出
        ...
        try {
            // 将状态设置为`STARTING_PREP`
            setStateInternal(LifecycleState.STARTING_PREP, null, false);
            // start逻辑，抽象方法，由组件自行实现
            startInternal();
            // 判断startInternal()方法执行结果
            // 如果是`FAILED`则直接stop
            // 如果不是`STARTING`则为非法状态，抛出异常结束
            // 如果是`STARTING`则说明流程正常，进行状态设置
    ...
    public final synchronized void stop() throws LifecycleException {
        ...
        try {
            // `FAILED`时直接触发BEFORE_STOP_EVENT事件，
            // 否则设置状态为STOPPING_PREP
            if (state.equals(LifecycleState.FAILED)) {
